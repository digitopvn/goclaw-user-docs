	...
	return (
		<Switch>
			<Match when={local.collapsible === 'none'}>
				<div
					class={cn('test bg-sidebar text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col', local.class)}
					{...others}
				>
					{local.children}
				</div>
...
const SidebarSeparator = <T extends ValidComponent = 'hr'>(props: SidebarSeparatorProps<T>) => {
	...
	return <Separator data-sidebar="separator" class={cn('bg-sidebar-border mx-2 w-auto', local.class)} {...others} />;
};
