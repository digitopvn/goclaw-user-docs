...
description: "Complete HTTP REST API reference for GoClaw"
---

# HTTP REST API Reference

GoClaw provides a full HTTP REST API running in parallel with the WebSocket RPC. All endpoints are served on the same port as the gateway server.
...
| Type | Format | Scope |
|------|--------|-------|
| Gateway token | Configured in `config.json` (`gateway.token` field) | Full Admin access |
| API key | `goclaw_` + 32 hex chars | Scoped by key permissions |

API keys are SHA-256 hashed before lookup — raw keys are never stored. Some endpoints accept tokens via the `?token=<token>` query parameter (for use in `<img>` and `<audio>` tags).
...
| `X-GoClaw-Agent-Id` | Agent identifier for scoped tasks |
...
CRUD for agent management. Requires `X-GoClaw-User-Id` header for multi-tenant context.
...
|--------|------|-------------|
| `POST` | `/v1/agents/{id}/wake` | Trigger agent externally (n8n, orchestrators) |
| `POST` | `/v1/agents/{id}/regenerate` | Regenerate agent config via LLM |
...
| `GET` | `/v1/agents/{id}/instances/{userID}/files` | List context files for a user |
...
Session details are managed primarily via WebSocket RPC (see `sessions.*` methods).
...
| `POST` | `/v1/tools/invoke` | Invoke a tool directly without the agent loop |
...
| `POST` | `/v1/mcp/servers/{id}/grants/agent` | Grant agent access |
...
| `POST` | `/v1/mcp/servers/{id}/grants/user` | Grant user access |
...
| `PUT` | `/v1/channel-instances/{id}` | Update instance config |
...
Agent resolution: `model` field with `goclaw:` or `agent:` prefix, then `X-GoClaw-Agent-Id` header, then agent `"default"`.
...
Alternative protocol compatible with the OpenAI Responses API. Same authentication, returns structured response objects (`response.started`, `response.delta`, `response.done`).

---

## Rate Limiting

Token bucket rate limiting per user or IP. Configured via `gateway.rate_limit_rpm` (0 = off, > 0 = on).
...
| `INVALID_REQUEST` | Missing or invalid fields in request |
...
Error responses include a `retryable` (boolean) field and `retryAfterMs` (integer) to guide client retry logic.
