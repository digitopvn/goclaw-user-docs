...
**Vi tri mac dinh:**
- Bien moi truong `GOCLAW_CONFIG` chi dinh duong dan tuong minh
- Mac dinh: `~/.goclaw/data/config.json` (Desktop), hoac `./config.json` (Standard)

**Nguyen tac:**
- Secrets (API keys, tokens, DSN) KHONG BAO GIO luu trong `config.json`
- Dung `.env.local` hoac bien moi truong cho tat ca secrets
- Config co the reload nong qua `config.apply` hoac `config.patch` (WebSocket)
...
Cau hinh moi kenh (channel) tich hop.
...
| `mention_mode` | string | `"strict"` | `"strict"` (chi khi mention), `"yield"` (tru khi bot khac duoc mention) |
...
| `reaction_level` | string | `"off"` | `"off"`, `"minimal"`, `"full"` — emoji reactions |
| `media_max_bytes` | int64 | `20MB` | Kich thuoc toi da tai media (bytes) |
...
Cau hinh LLM-based compaction cho pending group messages.
...
Cau hinh LLM providers. API keys luon doc tu env vars — KHONG luu trong `config.json`.
...
| `cohere` | `api_key`, `api_base` | Cohere |
| `perplexity` | `api_key`, `api_base` | Perplexity |
| `dashscope` | `api_key`, `api_base` | Alibaba DashScope (Qwen) |
| `bailian` | `api_key`, `api_base` | Alibaba Bailian |
...
Dieu khien tinh kha dung va chinh sach cua tools.

| Truong | Kieu | Mo ta |
|--------|------|-------|
| `profile` | string | Profile toan cuc: `"minimal"`, `"coding"`, `"messaging"`, `"full"` |
...
| Truong | Mac dinh | Mo ta |
|--------|----------|-------|
| `security` | `"full"` | `"deny"` (chan tat ca), `"allowlist"`, `"full"` (cho phep tat ca) |
| `ask` | `"off"` | `"off"`, `"on-miss"`, `"always"` — hoi user truoc khi chay |
...
| `headless` | `false` | Chay Chrome headless (bo qua khi dung `remote_url`) |
| `remote_url` | `""` | CDP endpoint cho remote Chrome, vi du `"ws://chrome:9222"` |
...
Map ten server → MCPServerConfig:
  ...
  tools: {
    mcp_servers: {
      "my-server": {
        ...
        env: {"API_KEY": "xxx"},
        tool_prefix: "my_",        // tranh xung dot ten
        timeout_sec: 60,
        enabled: true
      }
    }
...
Dieu khien hanh vi session.

| Truong | Kieu | Mac dinh | Mo ta |
|--------|------|----------|-------|
| `scope` | string | `"per-sender"` | `"per-sender"`: moi sender co session rieng; `"global"`: tat ca dung chung |
| `dm_scope` | string | `"per-channel-peer"` | Pham vi DM session (xem bang ben duoi) |
| `main_key` | string | `"main"` | Key suffix cho main session (khi `dm_scope="main"`) |
...
| `tts.elevenlabs` | `api_key`, `base_url`, `voice_id` (mac dinh `"pMsXgVXv3BLzUgSXRplE"`), `model_id` (mac dinh `"eleven_multilingual_v2"`) |
| `tts.edge` | `enabled`, `voice` (mac dinh `"en-US-MichelleNeural"`), `rate` |
| `tts.minimax` | `api_key`, `group_id`, `api_base`, `model` (mac dinh `"speech-02-hd"`), `voice_id` |

---

## 8. Section: agents.defaults

Gia tri mac dinh ap dung cho tat ca agents.
...
| `agent_type` | `"open"` | `"open"` (context rieng theo user) hoac `"predefined"` (shared context) |

---

## 9. Bien Moi Truong

Secrets va cau hinh nhay cam chi duoc doc tu env vars, khong bao gio tu `config.json`:
...
| `GOCLAW_STORAGE_BACKEND` | `"postgres"` hoac `"sqlite"` (mac dinh `"postgres"`) |
| `GOCLAW_SQLITE_PATH` | Duong dan SQLite DB (mac dinh `{dataDir}/goclaw.db`) |
