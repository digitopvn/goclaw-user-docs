...
| `teams.tasks.comments` | Liet ke comments | Admin |
| `teams.tasks.events` | Liet ke task events | Admin |
...
| `teams.events.list` | Liet ke team task events (phan trang) | Admin |
...
| `config.apply` | Thay the toan bo config (su dung optimistic locking qua `baseHash`) | Admin |
...
| Method | Mo ta | Role toi thieu |
|--------|-------|---------------|
| (Quan ly MCP qua HTTP API — xem tai lieu 30-api-reference.md) | — | — |
